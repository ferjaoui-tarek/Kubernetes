...
1. Create an IAM user/role  with Route53, EC2, IAM and S3 full access
    ```
   IAM--> ROLE --> EC2 --> Next permissions --> check ( AmazonEC2FullAccess, IAMFullAccess, AmazonS3FullAccess, AmazonRoute53FullAccess )
    ```
...
1. create an S3 bucket 
   ```sh
    aws s3 mb s3://dev.k8s.trkfrk.in
   ```
1. Expose environment variable:
   ```sh 
    export KOPS_STATE_STORE=s3://dev.k8s.trkfrk.in
   ```
...
1. Expose the deployment as service. This will create an ELB in front of those 2 containers and allow us to publicly access them:
 ...
 1. To delete cluster
    ```sh
     kops delete cluster dev.k8s.trkfrk.in --yes
    ```
